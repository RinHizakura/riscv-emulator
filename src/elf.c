...
static Elf64_Ehdr read_elf_header(char *elf_file)
{
...
static Elf64_Shdr read_section_header(char *elf_file,
                                      Elf64_Ehdr elf_header,
                                      int offset)
{
    Elf64_Shdr sec_header;
    char *sec = elf_file + elf_header.e_shoff + elf_header.e_shentsize * offset;
    memcpy(&sec_header, sec, sizeof(Elf64_Shdr));
    return sec_header;
}

static Elf64_Phdr read_program_header(char *elf_file,
                                      Elf64_Ehdr elf_header,
                                      int offset)
{
...
bool elf_parser(riscv_elf *elf, const char *filename)
{
    ...
    char *elf_file = malloc(sz);
    size_t read_size = fread(elf_file, sizeof(uint8_t), sz, fp);
    ...
    Elf64_Ehdr elf_header = read_elf_header(elf_file);

    Elf64_Shdr sectab_sec_header =
        read_section_header(elf_file, elf_header, elf_header.e_shstrndx);
    char *sectab = elf_file + sectab_sec_header.sh_offset;

    printf("%16s %14s %14s\n", "Name", "Start", "end");

    bool first_text = true;
    for (int i = 0; i < elf_header.e_shnum; i++) {
        Elf64_Shdr sec_header = read_section_header(elf_file, elf_header, i);
        ...
        if (sec_header.sh_type == SHT_SYMTAB ||
            ...
            Elf64_Shdr symtab_sec_header =
                read_section_header(elf_file, elf_header, sec_header.sh_link);
            char *symtab = elf_file + symtab_sec_header.sh_offset;
    ...
    for (int i = 0; i < elf_header.e_phnum; i++) {
        Elf64_Phdr prog_header = read_program_header(elf_file, elf_header, i);
